...
| `tool/code-dispatch-start` … `tool/code-dispatch` | one nested `tool` span per call a `run_code` program made |
...
**The scope names this package, not an SDK.** Batches arrive under the instrumentation scope `dsh-litefuse-plugin`. The ingest copies a span's whole raw attribute map into each observation's metadata as `attributes` unless that name begins with `langfuse-sdk`; claiming the prefix suppresses the copy, and one release did, but the scope is an identity field and this is not a Langfuse SDK. The copy costs noise rather than correctness — every attribute in it is already a first-class field, so nothing is lost or wrong, only duplicated in a form that re-encodes the JSON strings `model.parameters` and `usage_details` are required to be. The accurate fix belongs on the ingest side, which already receives `x-langfuse-ingestion-version: 4` from this exporter: the documented signal that a client sends complete spans inline, and one that is true of any custom exporter making it rather than of a single name prefix.
